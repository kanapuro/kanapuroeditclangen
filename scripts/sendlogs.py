...
def verify_token(token: str) -> bool:
    ...
    """
    with requests.get("https://fringe.silverpelt.lvna.me/logtoken/" + token) as response:
...
def upload_logs(token: str, progress: UITextBoxTweaked, callback: callable):
    ...
    req = requests.post("https://fringe.silverpelt.lvna.me/logs/", data=ujson.dumps(files), headers={'Content-Type': 'application/json', 'token': token})
