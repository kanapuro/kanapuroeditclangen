...
class Patrol:
    ...
    def calculate_success(self, success_outcome: PatrolOutcome, fail_outcome: PatrolOutcome) -> Tuple[PatrolOutcome, bool]:
        ...
        for kitty in self.patrol_cats:
            if hasattr(kitty, 'skills') and kitty.skills:
                hits = kitty.skills.check_skill_requirement_list(success_outcome.stat_skill)
            else:
                hits = 0  # or some default
                print(f"WARNING: Cat {kitty.ID} ({kitty.name}) has no skills attribute!")
            success_chance += (
            ...
            if hasattr(kitty, 'skills') and kitty.skills:
                ...
                hits = 0
                print(f"WARNING: Cat {kitty.ID} ({kitty.name}) has no skills attribute for fail check!")
            success_chance -= (
