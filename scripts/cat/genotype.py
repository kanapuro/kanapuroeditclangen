...
class Genotype:
    ...
    def fromJSON(self, jsonstring):
        ...
        self.white_pattern = jsonstring.get("white_pattern", [])
    ...
    def AltGenerator(self, special=None):
        ...
        for i in range(0, 4):
            self.unders_ruf += choice(self.odds["rufousing_kittypet"])
            self.unders_rufsum += int(self.unders_ruf[i])
    ...
    def KitGenerator(self, par1, par2=None, par3=None):
        # Normalize parent objects to Genotype-like instances
        try:
            par2 = par2.genotype
        except Exception:
            pass
        try:
            par1 = par1.genotype if hasattr(par1, 'genotype') else par1
        except Exception:
            pass

        # Create and initialize fallback parents when missing
        if not par2:
            logger.debug("No second parent genotype given")
            par2 = Genotype(self.odds, self.ban_genes, 'no chimeras')
            # Initialize alleles so inheritance can sample safely
            par2.Generator()
        if not par1:
            logger.debug("No first parent genotype given")
