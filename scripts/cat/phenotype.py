...
class Phenotype(Genotype):
    ...
    def PhenotypeOutput(self, pattern=None, gender=None, chimera=False):
        ...
        # Apply pelt customizations if they override the genetic appearance
        # This ensures that customized appearance shows in the genetics display
        if hasattr(self, '_pelt') and self._pelt:
            pelt = self._pelt
            # Override colour if customized
            if pelt.colour and pelt.colour != "WHITE":
                self.colour = pelt.colour
            # Override pattern if customized  
            if pelt.pattern:
                self.tabby = pelt.pattern if isinstance(pelt.pattern, str) else (pelt.pattern[0] if pelt.pattern else "")
            # Override tortie pattern if customized
            if pelt.tortiepattern:
                tortie_pat = pelt.tortiepattern if isinstance(pelt.tortiepattern, str) else (pelt.tortiepattern[0] if pelt.tortiepattern else "")
                if tortie_pat and tortie_pat != "Single":
                    self.tortie = "tortie "
        ...
        # Determine breed label using merged signature (genotype, phenotype, config)
        try:
            breed = find_my_breed(self, self, game.config)
        except TypeError:
