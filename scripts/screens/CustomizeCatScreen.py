...
def create_dropdown(pos, size, options, selected_option, style=None):
    # Validate selected_option exists in options; fall back safely if not.
    try:
                ...
                if sel not in opt_list:
                    ...
                    for o in opt_list:
                        try:
                            ...
                            continue
                    sel = found if found else opt_list[0]
            else:
                if sel not in opt_list:
                    found = None
                    for o in opt_list:
        ...
        return UIDropDownMenu(options, sel, ui_scale(Rect(pos, size)), object_id=f"#{style}", manager=MANAGER)
    except Exception:
...
class CustomizeCatScreen(Screens):
    def __init__(self, name=None):
        ...
        self.accessories = list(
            dict.fromkeys(Pelt.plant_accessories + Pelt.wild_accessories +
    ...
    def handle_back_button(self):
        ...
        # Refresh phenotype display by calling PhenotypeOutput to cache the appearance info
        # This ensures genetics tab shows accurate phenotype based on customized pelt attributes
        if hasattr(self.the_cat, 'phenotype') and self.the_cat.phenotype:
            try:
                self.the_cat.phenotype.PhenotypeOutput()
            except Exception as e:
                # Silently catch errors in phenotype refresh - it's not critical
                pass
