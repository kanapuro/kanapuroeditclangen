...
for rel in rel_types:
    with open(os.path.join(base_path, rel, "increase.json"), "r", encoding="utf-8") as read_file:
        loaded_list = ujson.loads(read_file.read())
        INTERACTION_MASTER_DICT[rel]["increase"] = create_interaction(loaded_list)
    with open(os.path.join(base_path, rel, "decrease.json"), "r", encoding="utf-8") as read_file:
        loaded_list = ujson.loads(read_file.read())
        INTERACTION_MASTER_DICT[rel]["decrease"] = create_interaction(loaded_list)

with open(os.path.join(base_path, "neutral.json"), "r", encoding="utf-8") as read_file:
    loaded_list = ujson.loads(read_file.read())
